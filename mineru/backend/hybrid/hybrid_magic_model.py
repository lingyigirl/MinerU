    ...
    def __init__(self,
        ...
        for index, block_info in enumerate(self.page_blocks):
            try:
                ...
                # ========== 核心：旋转判断与坐标修正 ==========
                block_angle = normalize_angle(block_angle)

                # 旋转 bbox 坐标
                if block_angle != 0:
                    block_bbox = rotate_bbox(block_bbox, block_angle, self.width, self.height)

                 # 2. 旋转 page_text_inline_formula_spans 里的所有 span（关键！你之前缺这个）
                rotated_spans = []
                for span in page_text_inline_formula_spans:
                    s_bbox = span["bbox"]
                    if block_angle != 0:
                        s_bbox = rotate_bbox(s_bbox, block_angle, self.width, self.height)
                    rotated_span = {**span, "bbox": s_bbox}
                    rotated_spans.append(rotated_span)

                page_text_inline_formula_spans = rotated_spans
