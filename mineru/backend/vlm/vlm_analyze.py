...
def doc_analyze(
    ...
    **kwargs,
    ...
    try:
        ...
        try:
            for window_index, window_start in enumerate(range(0, page_count, effective_window_size or 1)):
                ...
                try:
                    ...
                    # 转成 RGB 防止透明通道报错
                    images_pil_list = [img.convert("RGB") for img in images_pil_list]

                    # 保存成一个 PDF
                  #  first_img = images_rgb[0]
                 ...
                 #   )
                    ...
                    with predictor_execution_guard(predictor):
                        ...
                        print('=====================window_results==================',window_results)
        ...
        doc_closed = True
        return middle_json, results,final_bytes
    finally:
        if not doc_closed:
            close_pdfium_document(pdf_doc)
    ...
    try:
        ...
        try:
            for window_index, window_start in enumerate(range(0, page_count, effective_window_size or 1)):
                ...
                try:
                    # 转成 RGB 防止透明通道报错
                    images_pil_list = [image_dict["img_pil"].convert("RGB") for image_dict in images_list]
