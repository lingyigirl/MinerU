...
from mineru.utils.pdfium_guard import rewrite_pdf_bytes_with_pdfium, open_pdfium_document
...
def _process_output(
    ...
    if final_bytes is not None and final_bytes != '':
        # 保存旋转后的pdf文件
        with open(f"{local_md_dir}/{pdf_file_name}_rotate.pdf", "wb") as f:
            f.write(final_bytes)

    logger.debug(f"local output dir is {local_md_dir}")

def _process_output_bak(
        pdf_info,
        pdf_bytes,
        pdf_file_name,
        local_md_dir,
...
def _process_pipeline(
    ...
    def run_output_task(doc_index, middle_json, model_list,final_bytes):
        ...
        try:
            _process_output(
                ...
                f_make_md_mode, middle_json, final_bytes,model_list, process_mode="pipeline"
            )
    ...
    with ThreadPoolExecutor(max_workers=1) as output_executor:
        def on_doc_ready(doc_index, model_list, middle_json, ocr_enable,final_bytes):
            logger.debug(
                f"Pipeline doc ready: doc{doc_index} pages={len(middle_json['pdf_info'])} output_submitted=1"
            )
            future = output_executor.submit(run_output_task, doc_index, middle_json, model_list,final_bytes)
...
async def _async_process_vlm(
    ...
    for idx, pdf_bytes in enumerate(pdf_bytes_list):
        ...
        middle_json, infer_result, final_bytes = await aio_vlm_doc_analyze(
        ...
        _process_output(
            ...
            md_writer, f_draw_layout_bbox, f_draw_span_bbox, f_dump_orig_pdf,
            f_dump_md, f_dump_content_list, f_dump_middle_json, f_dump_model_output,
            f_make_md_mode, middle_json, final_bytes,infer_result,process_mode="vlm"
        )
...
def _process_vlm(
    ...
    for idx, pdf_bytes in enumerate(pdf_bytes_list):
        ...
        middle_json, infer_result,final_bytes = vlm_doc_analyze(
            pdf_bytes, image_writer=image_writer, backend=backend, server_url=server_url, **kwargs,
        )
...
def _process_hybrid(
    ...
    for idx, (pdf_bytes, lang) in enumerate(zip(pdf_bytes_list, h_lang_list)):
        ...
        middle_json, infer_result, _vlm_ocr_enable,final_bytes = hybrid_doc_analyze(
            pdf_bytes,
...
async def _async_process_hybrid(
    ...
    for idx, (pdf_bytes, lang) in enumerate(zip(pdf_bytes_list, h_lang_list)):
        ...
        middle_json, infer_result, _vlm_ocr_enable,final_bytes = await aio_hybrid_doc_analyze(
            pdf_bytes,
            image_writer=image_writer,
            backend=backend,
            parse_method=parse_method,
    ...
    for i, file_bytes in enumerate(pdf_bytes_list):
        ...
        if file_suffix in docx_suffixes:
            ...
            _process_output(
                ...
                f_dump_md, f_dump_content_list, f_dump_middle_json, f_dump_model_output,
                f_make_md_mode, middle_json, final_bytes,infer_result, process_mode="docx"
            )
...
if __name__ == "__main__":
    ...
    pdf_path = "C:/Users/薛/Desktop/1111.pdf"
    #pdf_path = "D:/download/linuxdownfile/2.pdf"
    #pdf_path = "D:/testcode/11112_clean.png"
